...
def getTargetChannelId() -> int:
    # 質問させたいチャンネル(対象チャンネル)
    target_channel = {'id': 660097617266737153, 'position': 99999999}
    # ***********************************************************
    # 指定カテゴリ(対象チャンネルが含まれたカテゴリ)の名前

    category_id = 0s  # カテゴリidを指定
    target_category_name = client.get_channel(category_id).name

    # ***********************************************************
    # 指定したサーバにある全てのTextチャンル一覧
    all_channels = client.get_guild(642398717634412575).text_channels
...
client.run('NjY1MjE5NDI0MDc2MzAwMjk4.Xhib5w.7_xlFRNON0aTvJVWMglY_JBK-MA')
